...
function shortUrl(u?: string | null) {
  ...
  try {
    ...
    // exibe algo como: s.shopee.com.br/abc123
    ...
    <div className="relative max-w-6xl mx-auto px-4 pb-16">
      ...
      <div className="mt-4 overflow-x-auto">
        <div className="min-w-[1100px] rounded-2xl border border-zinc-200 overflow-hidden bg-white">
          {/* cabeçalho */}
          <div className="grid grid-cols-[210px_150px_1fr_320px_120px_120px] items-center bg-zinc-50/60 px-4 py-3 text-xs font-medium text-zinc-600">
            <div>Quando</div>
            <div>Plataforma</div>
            <div>Legenda</div>
            <div>Link</div>
          ...
          {loading ? (
            ...
            rows.map((r) => (
              <div
                key={r.id}
                className="grid grid-cols-[210px_150px_1fr_320px_120px_120px] items-center px-4 py-3 border-t border-zinc-200/70"
              >
                {/* quando */}
                ...
                <div className="text-sm text-zinc-800 pr-4">
                  {r.caption ? (
                ...
                {/* link curto (uma linha com truncation) */}
                <div className="text-sm pr-4">
                  {r.shortlink || r.url_canonical ? (
                    <a
                      href={r.shortlink || r.url_canonical!}
                      target="_blank"
                      rel="noreferrer"
                      className="text-zinc-800 underline underline-offset-2 block truncate"
                      title={r.shortlink || r.url_canonical || undefined}
                    >
                      {shortUrl(r.shortlink || r.url_canonical)}
                    </a>
                  ) : (
                    <span className="text-zinc-500">—</span>
                  )}
